...
range_to_fit = 1
path = r"Z:\Tweezer\People\Imogen\GitHub\slm\images\2021\December\18\Measure 41"
...
# #fitting gaussians
# #main_plot_ax.plot(amplitude_range[range_to_fit::]*1000,y(amplitude_range[range_to_fit::]*1000),label='Gaussian Fit for beam waist in x')
# #main_plot_ax.plot(amplitude_range[range_to_fit::]*1000,y_y(amplitude_range[range_to_fit::]*1000),label='Gaussian Fit for beam waist in x')
...
print(min(wx))
print(min(wy))
z_x = np.where(wx== min(wx))
z_y = np.where(wy == min(wy))
print(z_y)
print(z_x)
