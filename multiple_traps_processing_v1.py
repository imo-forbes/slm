...
measurements = np.arange(26,27,1)
...
def main(): 
    ...
    #set path to where images are saved
        ...
        for image_no in track(range(0,31,1)):
            image = cv2.imread(r"Z:\Tweezer\People\Imogen\GitHub\slm\images\2022\January\30\Measure {}\{}.png".format(measure_value, image_no),cv2.IMREAD_GRAYSCALE)
        ...
        for beam_no in range(0,3,1):
            ...
            print(wx_2)
            print(wx_3)
            wx_average = (wx_1[0:9]+ wx_2[0:9]+ wx_3[0:9])/3
            wx_std = np.std(np.vstack([wx_1[0:9], wx_2[0:9], wx_3[0:9]]), axis=0)
            ...
            wy_average = (wy_1[0:9] + wy_2[0:9] + wy_3[0:9])/3
            wy_std = np.std(np.vstack([wy_1[0:9], wy_2[0:9], wy_3[0:9]]), axis=0)
            ...
            plt.figure()
            plt.errorbar(amplitude_range[0:9]*1000, wx_average, yerr=wx_standard_err[0:9], marker='o', linestyle='', label='Mean beam waist in x for beam' +str(beam_no+1), color='blue')
            plt.errorbar(amplitude_range[0:9]*1000, wy_average, yerr=wy_standard_err[0:9], marker='o', linestyle='',  label= 'Mean beam waist in y', color='orange')
